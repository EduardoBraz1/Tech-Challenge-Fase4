# Streamlit app para visualizar os dados de obesidade e prever obesidade com base em dados de entrada do usuário
...
if st.button("Realizar Previsão", type="primary", width="stretch"): # Botão para realizar a previsão, com estilo primário e largura total do contêiner
    ...
    try:
        url_api = "http://api:5000/predict" # URL da API para realizar a previsão
        response = requests.post(url_api, json=input_data) # Enviando uma solicitação POST para a API com os dados de entrada em formato JSON

        if response.status_code == 200:
            result = response.json() # Convertendo a resposta da API para um dicionário Python
            diagnostico = result["data"]["prediction"] # Extraindo a previsão do diagnóstico de obesidade da resposta da API
            ...
            st.success(f"**Diagnóstico Preditivo:** {traducao.get(diagnostico, diagnostico)}") # Exibindo o resultado da previsão em uma mensagem de sucesso, traduzindo o diagnóstico para uma forma mais compreensível para o usuário
            st.balloons() # Exibindo balões de celebração para tornar a experiência do usuário mais divertida e envolvente
        else:
            st.error(f"Erro na API. Detalhes: {response.text}") # Exibindo uma mensagem de erro caso a resposta da API não seja bem-sucedida, sugerindo ao usuário verificar a API para resolver o problema

    except Exception as e:
        st.error(f"Não foi possível conectar à API. Garanta que ela está rodando no terminal! Erro: {e}") # Exibindo uma mensagem de erro caso haja um problema ao conectar à API, sugerindo ao usuário garantir que a API esteja rodando no terminal e exibindo o erro específico para ajudar na resolução do problema